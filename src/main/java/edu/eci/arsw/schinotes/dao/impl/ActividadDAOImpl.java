...
public class ActividadDAOImpl implements ActividadDAO {
    ...
    public List<Actividad> loadActividadesByGroup(int grupoId, String nombreHorario) throws SchiNotesException {
        String sql = "SELECT a.id,a.nombre,a.descripcion,to_char(a.fecha_creacion,'YYYY-MM-DD') as fecha_creacion,to_char(a.hora_hora, 'HH24:MI:SS') as hora_hora,a.hora_dias_por_horario_dia_nombre,a.hora_dias_por_horario_horario_id, to_char(a.hora_fin, 'HH24:MI:SS') as hora_fin\n"
                + "FROM grupo g JOIN horario h ON (g.horario_id=h.id) JOIN actividad_por_horario aph ON (h.id=aph.horario_id)\n"
                + "JOIN actividad a ON (a.id=aph.actividad_id)\n" + "WHERE g.identificacion = ? AND h.nombre = ?";
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(sql, new Object[] { grupoId, nombreHorario });
        List<Actividad> actividades = new ArrayList<>();
        for (Map<String, Object> row : rows) {
            Actividad actividad = new Actividad();
            actividad.setId((int) row.get("id"));
